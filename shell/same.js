...
const JSESSIONID = '' // cookie 需登录网站获取 例 8DBA06CA87781260C2023475912DF745
const username = '' // 用户名 例 zhuyucai
const urole = '' // 不知道 建议登录网站获取下 例 %E5%85%B6%E4%BB%96%2C
const url = 'http://58.246.39.26:15936' // 根据实际地址更改
...
const curlStr = _.range(startToEnd).map((i) => {
  ...
  if ( dayOfweek < 6 ) {
    return [`curl '${url}/Artimes/app/addorEdit' `,
      `-H 'Cookie: JSESSIONID=${JSESSIONID}; password=; rempwd=; username=${username}; urole=${urole}' `,
